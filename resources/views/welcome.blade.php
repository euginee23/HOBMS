...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="min-h-screen bg-white antialiased dark:bg-zinc-900">
        ...
        <section class="relative flex min-h-screen items-center justify-center overflow-hidden pt-16">
            <div class="absolute inset-0 bg-gradient-to-br from-blue-50 via-white to-amber-50 dark:from-zinc-900 dark:via-zinc-800 dark:to-zinc-900"></div>
            <div class="relative mx-auto max-w-7xl px-6 py-24 text-center">
                <h1 class="text-5xl font-bold tracking-tight text-zinc-900 sm:text-6xl lg:text-7xl dark:text-white">
                    Welcome to <span class="text-blue-600 dark:text-blue-400">HOBMS</span>
                </h1>
                <p class="mx-auto mt-6 max-w-2xl text-lg text-zinc-600 dark:text-zinc-400">
                    Hotel Online Booking Management System. Experience comfort, luxury, and exceptional service. Book your perfect room today.
                </p>
                <div class="mt-10 flex items-center justify-center gap-4">
                    <a href="{{ route('rooms.index') }}" class="rounded-lg bg-blue-600 px-8 py-3 text-sm font-semibold text-white shadow-sm hover:bg-blue-500" wire:navigate>
                        Browse Rooms
                    </a>
                    <a href="{{ route('portal.lookup') }}" class="rounded-lg border border-zinc-300 bg-white px-8 py-3 text-sm font-semibold text-zinc-700 shadow-sm hover:bg-zinc-50 dark:border-zinc-600 dark:bg-zinc-800 dark:text-zinc-300 dark:hover:bg-zinc-700" wire:navigate>
                        Track Booking
                    </a>
                </div>
        ...
        <section class="border-t border-zinc-200 bg-zinc-50 py-24 dark:border-zinc-700 dark:bg-zinc-800">
            <div class="mx-auto max-w-7xl px-6">
                <div class="text-center">
                    <h2 class="text-3xl font-bold tracking-tight text-zinc-900 dark:text-white">Our Rooms</h2>
                    <p class="mt-4 text-zinc-600 dark:text-zinc-400">Choose from our carefully curated selection of rooms</p>
                </div>
                ...
                <div class="mt-12 grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
                    @foreach(\App\Models\RoomCategory::active()->get() as $category)
                        <div class="group overflow-hidden rounded-xl border border-zinc-200 bg-white shadow-sm transition hover:shadow-md dark:border-zinc-700 dark:bg-zinc-900">
                            <div class="flex h-48 items-center justify-center bg-gradient-to-br from-blue-100 to-blue-50 dark:from-blue-900/20 dark:to-zinc-800">
                            ...
                            <div class="p-6">
                                ...
                                <p class="mt-2 text-sm text-zinc-600 dark:text-zinc-400">{{ Str::limit($category->description, 100) }}</p>
                                <div class="mt-4 flex items-center justify-between">
                                    ...
                                    <span class="text-sm text-zinc-500 dark:text-zinc-400">Up to {{ $category->max_capacity }} guests</span>
                                </div>
                                <a href="{{ route('rooms.show', $category->slug) }}" class="mt-4 inline-flex w-full items-center justify-center rounded-lg border border-blue-600 px-4 py-2 text-sm font-medium text-blue-600 hover:bg-blue-50 dark:border-blue-400 dark:text-blue-400 dark:hover:bg-blue-900/20" wire:navigate>
                                    View Details
                                </a>
                            </div>
                        </div>
        ...
        <section class="border-t border-zinc-200 py-24 dark:border-zinc-700">
            <div class="mx-auto max-w-2xl px-6 text-center">
                <h2 class="text-3xl font-bold tracking-tight text-zinc-900 dark:text-white">Already Booked?</h2>
                <p class="mt-4 text-zinc-600 dark:text-zinc-400">Track your booking status using your reference number</p>
                <div class="mt-8">
                    <a href="{{ route('portal.lookup') }}" class="inline-flex items-center gap-2 rounded-lg bg-zinc-900 px-8 py-3 text-sm font-semibold text-white shadow-sm hover:bg-zinc-700 dark:bg-white dark:text-zinc-900 dark:hover:bg-zinc-200" wire:navigate>
                        <svg class="size-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="m21 21-5.197-5.197m0 0A7.5 7.5 0 1 0 5.196 5.196a7.5 7.5 0 0 0 10.607 10.607Z"/></svg>
                        Track My Booking
                    </a>
                </div>
            </div>
